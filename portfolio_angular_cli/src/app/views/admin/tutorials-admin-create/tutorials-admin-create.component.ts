...
@Component({
  selector: 'app-tutorials-admin-create',
  templateUrl: './tutorials-admin-create.component.html',
  styleUrls: ['./tutorials-admin-create.component.css']
})
export class TutorialsAdminCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.authenticationService.authentication("tutorialsAdminCreate")   
  }
  ...
  saveDraft(): void {
    ...
    this.storeData.id = this.tutorialsService.TUTORIALS_DATA_SERVICE.length + 1
    ...
    if (this.storeData.title == "") {
      this.storeData.title = "unknown_PostId_" + this.storeData.id
      this.storeData.subtitle = "Não informado"
    }
    console.log('-> Concluída verificação do campo title não nulo')
    console.log(this.storeData)
    this.tutorialsService.createData(this.storeData)
    ...
    console.log('-> Router para página de Posts do tutorials')
